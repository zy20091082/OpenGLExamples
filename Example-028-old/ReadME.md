...
The <i>'Example-028 (Old Mode)'</i> Test draws a polyline (in <i>'red'</i>), formed by an arbitrary number of samples, in an <A href="http://www.opengl.org"><i>OpenGL</i></A> window. The polyline of interest approximates the <i>'Cosine-like'</i> curve, defined in the range <i>'[-pi,+pi]'</i>. In particular, the <i>'Cosine-like'</i> curve is defined in the same spirit of the <i>'Cosine'</i> curve, but it requires an exponent <i>'n'</i> for computing the cosine function to the <i>'n'</i>-th, as follows:<center>
<pre>x(t) = t, y(t) = cos^n (t)</pre></center><p>with <i>'n'>0</i> and <i>'t'</i> in <i>'[-pi,pi]'</i>.<p>This test also provides a very basic interaction mechanism with the user, which must provide the exponent <i>'n'</i>, the semiaxis <i>'Rx'</i> and <i>'Ry'</i>, as well as the center <i>'(xc,yc)'</i>. The user can also:<p><ul>
...
In particular, this test consists of the <i>'Example-028-old'</i> executable file (in the <i>'bin'</i> folder), starting uniquely from the <i>'main.cpp'</i> C++ source file (in the <i>'src'</i> folder).<p><hr><p>
