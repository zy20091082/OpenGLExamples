...
The <i>'Example-028 (Old Mode)'</i> Test draws a polyline (in <i>'red'</i>), formed by an arbitrary number <i>'n'</i> of vertices and edges, in an <A href="http://www.opengl.org"><i>OpenGL</i></A> window. The polyline of interest approximates the <i>'Cosine-like'</i> curve in the range <i>'[-pi,+pi]'</i>. In particular, any <i>'Cosine-like'</i> curve is defined in the same spirit of the <i>'Cosine'</i> curve, but it requires an exponent <i>'k'</i> for computing the cosine function to the <i>'k'</i>-th. This curve is defined as follows:<center><pre>x(t) = t, y(t) = cos ^ k (t)</pre></center><p>for any <i>'k'>0</i>, and for every <i>'t'</i> in <i>'[-pi,pi]'</i>.<p>This test also provides a very basic interaction mechanism with the user. In fact, this latter must provide the exponent <i>'k'</i>, and can also:<p><ul>
