...
int main(int argc,char **argv)
{
	...
	cout<<"\tIt draws a polyline in 'red' (formed by an arbitrary number of samples), which approximates the 'Cosine-like' curve, defined in the ["<<-PI<<",+"<<PI<<"] range."<<endl;
	...
	if( (!cin) || (ee<=0) )
	{
		cout<<endl<<"\tPLEASE, INSERT A VALID VALUE (POSITIVE AND NOT NULL) FOR THE EXPONENT 'n' IN THE FUNCTION, DEFINING THE 'COSINE-LIKE' CURVE OF INTEREST."<<endl<<endl<<"\tTHIS PROGRAM IS CLOSING ..."<<endl<<endl;
		cout.flush();
		return EXIT_FAILURE;
	}
...
void initialize() 
{
	...
	num_samples=3;
	cout<<endl<<"\tInitially, the polyline, approximating the 'Cosine-like' curve with exponent 'n'="<<ee<<", is formed by "<<num_samples<<" samples (the minimum number as possible)."<<endl<<endl;
	cout.flush();
}
...
void draw()
{
	...
	glEnd();
	glFlush();
	cout<<"\tThe 'Cosine-like' curve of interest is currently approximated by a polyline with "<<num_samples<<" samples (thus with "<<num_samples<<" vertices and "<<num_samples<<" edges)."<<endl;
	cout.flush();
}
