...
/// The number <i>'n'</i> of the vertices (including the <i>'apex'</i>) in all approximations of interest for the <i>'Cone'</i> shape.
/**
 * This value, initially set to 'n=5', is the number 'n' of the vertices in all approximations of interest for the 'Cone' shape. Broadly speaking, it is the number 'n' 
 * of all vertices (including the 'apex') in the triangle fans of interest. The value of 'n' can be interactively modified by pressing the '+' and the '-' keys.
 */
...
int main(int argc,char **argv)
{
	...
	cout<<"'Cone' shape tapers smoothly from a flat base (often, but not necessarily,"<<endl;
	cout<<"\tdescribed by a 'Circle' shape) to a given point, called the 'apex' or the 'vertex'. In other words, the 'Cone' shape is formed by a ";
	cout<<"collection of lines, connecting the 'apex' to all of the points on a base, which does not contain"<<endl;
	cout<<"\tthe 'apex'. The 'axis' of the 'Cone' shape is the line, passing through the 'apex', about which the 'Cone' shape has a circular symmetry."<<endl<<endl;
	...
	cout<<" 'Cone' shape. The 'Cone' shape of interest is open, thus we consider only"<<endl;
	cout<<"\tthe boundary of the 'Circle' shape, described by the 'Circle' curve."<<endl<<endl;
	cout<<"\tIn this case, the 'Cone' shape of interest is described by a triangle fan (in 'red') with 'n' vertices (including the 'apex'). Thanks to its symmetry ";
	cout<<"properties, it is interesting to analyze (upon request) '6' views of the 'Cone'"<<endl;
	cout<<"\tshape, in particular:"<<endl<<endl;
	...
	cout<<"overlap and not be visible in a specific view. Finally, it also possible to"<<endl;
	cout<<"\trender (upon request) the 'wireframe' and the 'filled versions' of all triangles in the current approximation of the 'Cone' shape. These aspects are ";
	...
	cout<<"the 'r' key;"<<endl;
	cout<<"\t\t-) increase the number 'n' of all vertices (including the 'apex') in the triangle fans of interest by pressing the '+' key;"<<endl;
	cout<<"\t\t-) decrease the number 'n' of all vertices (including the 'apex') in the triangle fans of interest by pressing the '-' key."<<endl<<endl;
...
void initialize() 
{
	...
	cout<<"triangles in the triangle fans (formed by the minimum number 'n="<<num_samples<<"' as possible of the vertices, including the 'apex') in the 'View #";
	cout<<configuration<<"' drawn by using the ";
...
void draw()
{
	...
	if(mode==GL_FILL) cout<<"\tThe 'filled versions' of all ";
	if(mode==GL_LINE) cout<<"\tThe 'wireframe versions' of all ";
	cout<<"triangles in the triangle fans (formed by 'n="<<num_samples<<"' vertices, including the 'apex') in the 'View #"<<configuration<<"' are currently drawn by ";
	cout<<"using the ";
...
void manageKeys(unsigned char key, int x, int y)
{
	...
	switch (key)
	{
		...
		case '+':
			...
			/* The key is '+', thus we increase the number 'n' of the vertices (including the 'apex') in the triangle fans of interest! */
		...
		case '-':
			...
			/* The key is '-', thus we decrease (if it is possible) the number 'n' of the vertices (including the 'apex') in the triangle fans of interest! */
			if(num_samples>5) num_samples=num_samples-1;
			else
			{
				cout<<"\tThe minimum number 'n=5' of the vertices (including the 'apex') in the triangle fans of interest is reached, and it is not possible to ";
