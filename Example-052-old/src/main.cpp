...
int main(int argc,char **argv)
{
	...
	cout<<" construction, it is necessary to enumerate a sequence of '10' points with"<<endl;
	cout<<"\ttheir independent colors ('2'points and their colors are needed to be duplicated)."<<endl<<endl;
	cout<<"\tFor the sake of the efficiency, we exploit a centralized data organization, consisting of '2' matrices, such that their rows contain, respectively, the ";
	cout<<"Euclidean '3D' coordinates and the colors components for all '8' vertices"<<endl;
	cout<<"\t(with no redundancy). Hence, it is sufficient to access the locations of indices '0,...,7,0,1' in '2' matrices, mentioned above, in order to draw the ";
	cout<<"triangle strip of interest. This technique works perfectly, and it is easy to"<<endl;
	cout<<"\tbe customized for a different number of vertices, or for a different colors palette. In fact, it is sufficient to modify only the desired rows in the";
	cout<<" matrices of interest. These matrices may be precomputed in advance and reused"<<endl;
	cout<<"\tmany times, if necessary."<<endl<<endl;
	...
	cout<<"\tThis means that only a subset of all triangles in the triangle strip of interest are drawn. Broadly speaking, every index 'j' in '[0,n-1]' is generated ";
	cout<<"in order to access '2' rows of index 'j % 8' in the matrices of interest."<<endl<<endl;
...
void draw()
{
	/* We draw the portion of interest for the triangle strip, approximating the 'Square Annulus' shape, in the main OpenGL window by using the rendering
