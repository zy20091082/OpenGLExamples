...
</code> columns. Each row of this matrix corresponds to a <i>'stack'</i> along the <i>Z</i>-axis (similar to a latitudinal line), approximated by a triangle strip, and contains the Euclidean <code><i>'3D'</i></code> coordinates (a tuple of <code><i>'3'</i></code> floating-point values) for all <code><i>'2*(p+1)'</i></code>
for all <code><i>'2*(p+1)'</i></code> points in this triangle strip.
