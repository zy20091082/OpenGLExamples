...
/**
 * It is initially set to 'n=25', which is the minimum number 'n' of the vertices and the edges in the polyline of interest. It is interactively modified by pressing
 * the '+' and the '-' keys.
 */
...
int main(int argc,char **argv)
{
	...
	cout<<"\tx(t) = (1+t) * cos(t), y(t) = (1+t) * sin(t)"<<endl<<endl<<"\tfor every 't' in '[-6 * pi, 6 * pi]'."<<endl<<endl<<"\tThis test also provides a very basic";
	cout<<" interaction mechanism with the user, which can:"<<endl<<endl;
...
void manageKeys(unsigned char key, int x, int y)
{
	...
	switch (key)
	{
		...
		{
			cout<<"\tThe minimum number 'n=25' of the vertices and the edges in the polyline of interest is reached, and it is not possible to decrease again this ";
			cout<<"number."<<endl;
			cout.flush();
		}
