...
<li>the <i>'Variant #0'</i> of the <i>'Circular Annulus'</i> shape exploits <code><i>'2'</i></code> triangle fans, formed by <code><i>'n>4'</i></code> vertices (including their <i>'reference vertices'</i>). By construction, it is not possible to have <code><i>'n<5'</i></code>. Broadly speaking, <code><i>'2'</i></code> triangle fans describe, respectively, the <i>'inner'</i> disk (in <i>'blue'</i>) and the <i>'outer'</i> disk (in <i>'red'</i>) at different z-depths. These disks are described by the <i>'Circle'</i> shape, and are drawn by using the orthographic projection, such that their <i>'reference vertices'</i> are projected on the same point. Here, the user can choose to modify the rendering order of the <i>'inner'</i> and the <i>'outer'</i> disks by obtaining the following configurations:<p><ul>
