...
/// The setting for choosing the rendering order of the triangle fans in the <i>'Variant #0'</i> of the <i>'Circular Annulus'</i> shape.
/**
 * In the 'Variant #0' of the 'Circular Annulus' shape, 2 triangle fans, respectively in 'red' (the 'external disk') and 'blue' (the 'internal disk') at different 
 * z-depths are rendered without using the z-buffer (depth test). Thus, their rendering order is fundamental for drawing the resulting scene. There are 2 possible cases:
 *
...
int main(int argc,char **argv)
{
	...
	cout<<"disks, placed at different z-depths. The scene is drawn by using the"<<endl;
	cout<<"\t\t   orthographic projection, such that the centers for the 'Circle' shapes of interest are projected on the same point. Thus, they only seem to ";
	cout<<"approximate the 'Circular Annulus' shape. However, the rendering order of the"<<endl;
	cout<<"\t\t   'Circle' shapes may be modified upon request by the user. Thus, 2 configurations are possible, depending on which 'Circle' shape is rendered before";
	cout<<"in the scene. This means that the result is 'the same as' the"<<endl;
	cout<<"\t\t   'Circular Annulus' shape only if the 'external' disk is drawn before than the 'internal' disk (the 'Configuration #0'). Otherwise, only the ";
	cout<<"'external' disk is visible in the scene (the 'Configuration #1'). Both the"<<endl;
	cout<<"\t\t   'Circle' shapes of interest are approximated by a triangle fan of 'n' vertices (including the center)."<<endl<<endl;
	...
	cout<<"\t\t   always the same, despite the rendering order of the 'Circle' shapes. Also in this case, both the 'Circle' shapes of interest are approximated by a";
	cout<<" triangle fan of 'n' vertices (including the center)."<<endl<<endl;
	cout<<"\t\t2. The 'Variant #2' is a real approximation of the 'Circular Annulus' shape through a triangle strip (in 'red'), defined over 'n' pairs of the ";
	cout<<" corresponding vertices (including the center) between the approximations of"<<endl;
...
void initialize() 
{
	...
	mode=GL_FILL;
	odmode=0;
	cout<<"\tAt the beginning, all 'Circular Annulus' shapes are approximated by the ";
...
void manageKeys(unsigned char key, int x, int y)
{
	...
	switch (key)
	{
		...
		{
			cout<<"\tThe minimum number 'n=5' of the vertices in 2 triangle fans and of the vertices pairs in the triangle strip of interest is reached, and it is ";
			cout<<"not possible to decrease again this number."<<endl;
			cout.flush();
		}
...
void draw()
{
	 ...
	 * Here, the result depends on the rendering order of the 'Circle' shapes, which can be modified upon request by the user. Hence, the result is not necessarily 'the
	 * same as' the 'Circular Annulus' shape (thus also the 'Variant #1' of the 'Circular Annulus' shape), except when the 'Configuration #0' for the 'Variant #0' of
	 * the 'Circular Annulus' shape is exploited.
	 */
	 ...
	 * is exploited. Thus, the result does not depend on their rendering order, and it is only 'the same as' the 'Configuration #0' for the 'Variant #0' of the
	...
	cout<<"of several triangles in 2 triangle fans ('Configuration #"<<odmode<<"') and in a triangle strip with 'n="<<num_samples<<"' vertices and 'n=";
	cout<<num_samples<<"' vertices pairs, respectively."<<endl;
	cout.flush();
}
