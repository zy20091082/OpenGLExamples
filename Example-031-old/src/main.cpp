...
int main(int argc,char **argv)
{
	...
	cout<<"\t\t   by using the orthographic projection, such that their centers are projected on the same point. In this context, the rendering order of the 'Circle' shapes is modified upon request by the user. Hence, 2 different"<<endl;
	cout<<"\t\t   configurations [ 'Configuration #0 ] and [ 'Configuration #1' ] are possible, depending on which 'Circle' shape is rendered before. This means that the result is 'the same as' the 'Circular Annulus' shape only if"<<endl;
	cout<<"\t\t   the 'Circle' shape in 'red' is drawn before than the 'Circle' shape in 'blue' (i.e., the [ 'Configuration #0' ]). Both the 'Circle' shapes are approximated by a triangle fan of 'n' vertices (including the center)."<<endl<<endl;
