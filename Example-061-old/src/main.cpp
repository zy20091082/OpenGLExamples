...
int main(int argc,char **argv)
{
	...
	cout<<"\tor stairs, sits down on the top of the slide, and slides down the chute."<<endl<<endl;
	...
	cout<<"\tquadrilateral, which is the bottom basis in the 'xz' coordinate plane. Broadly speaking, the 'Playground Slide'shape is the '2D' shape, bounding this '3D'";
	cout<<" solid."<<endl<<endl;
	cout<<"\tThe parabolic profile of interest is approximated by a polyline with 'n' vertices and 'n' edges. This implies that each lateral side as well as the ";
	cout<<"bottom basis of the 'Playgrond Slide' shape can be approximated by '3' independent"<<endl;
	cout<<"\ttriangle strips. Instead, the frontward and the backward sides of the 'Playground Slide' shape are simply quadrilaterals, that can be triangulated. By ";
	cout<<"construction, these latter, together with the triangle strip, approximating the"<<endl;
	cout<<"\textrusion of the parabolic profile, can be merged into only one triangle strip. Specifically, the 'wireframe versions' of the triangles in these '4' ";
	cout<<"triangle grids (in 'green') are rendered by using the perspective projection."<<endl<<endl;
	...
	cout<<"This technique is used to group together several drawing instructions into only"<<endl;
	...
	cout<<"coordinates (thus, '3' floating-point values) for every point in '4' triangle strips of"<<endl;
	cout<<"\tinterest. These floating-point values are stored in a unique array, such that its content is recomputed when modifying the number 'n' of the samples in ";
	cout<<"the parabolic profile. In this case, the 'glMultiDrawElements()' function is"<<endl;
	...
	cout<<"the extrusion parameters for the 'Playground Slide' shape of interest, since they"<<endl;
...
void computePoints()
{
	...
	/* If we arrive here, then we complete 2 vertical basis (front and back). */
	ind=ind+1;
	inds[0][ind]=ind;
	vertices[3*ind]=5;
	vertices[3*ind+1]=0;
...
void manageKeys(unsigned char key, int x, int y)
{
	...
	switch (key)
	{
		...
		case '+':
			...
			/* The key is '+', thus we increase the number 'n' of the samples in the parabolic profile in the 'Playground Slide' shape. */
			destroyPoints();
		...
		case '-':
			...
			/* The key is '-', thus we reduce the number 'n' of the samples in the parabolic profile in the 'Playground Slide' shape. */
			if(num_samples>3) 
			{
				destroyPoints();
				num_samples=num_samples-1;
