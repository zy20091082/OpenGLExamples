...
The <i>'Example-008 (Old Mode)'</i> Test draws the <i>'filled versions'</i> of 2 quadrilaterals of different size and with a not empty intersection (partial overlap) in an <A href="http://www.opengl.org"><i>OpenGL</i></A> window, and provides a basic interaction mechanism with the user. In particular, the <i>'small'</i> quadrilateral may be completely covered by the <i>'large'</i> quadrilateral, depending on their rendering order. This latter corresponds to their color, as follows:<p><ul>
<li>the quadrilateral in <i>'red'</i> is rendered before in the scene;</li>
<li>the quadrilateral in <i>'blue'</i> is rendered after in the scene.</li></ul><p>The rendering order of these quadrilaterals corresponds to the following (viewing) configurations:<p><ul>
<li type=0>the <i>'large'</i> quadrilateral (in <i>'red'</i>) is rendered before, and the <i>'small'</i> quadrilateral (in <i>'blue'</i>) is rendered after in the <i>'(viewing) configuration #0'</i>. In this case, both the quadrilaterals are visible.<p></li>
<li type=1>The <i>'small'</i> quadrilateral (in <i>'red'</i>) is rendered before, and the <i>'large'</i> quadrilateral (in <i>'blue'</i>) is rendered after in the <i>'(viewing) configuration #1'</i>. In this case, only the <i>'large'</i> quadrilateral is visible.<p></li></ul><p>These (viewing) configurations can be activated upon request by pressing any among the <i>'0'</i> and the <i>'1'</i> keys. Likewise, the window of interest can be closed by pressing any among the <i>'Q'</i>, the <i>'q'</i>, and the <i>'Esc'</i> keys.<p>In particular, this test consists of the <i>'Example-008-old'</i> executable file (in the <i>'bin'</i> folder), starting uniquely from the <i>'main.cpp'</i> C++ source file (in the <i>'src'</i> folder).<p><hr><p>
