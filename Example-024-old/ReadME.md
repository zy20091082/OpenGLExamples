...
The <i>'Example-024 (Old Mode)'</i> Test draws several versions (in <i>'red'</i>) of a quadstrip (formed by an arbitrary number of quadrilaterals) in an <A href="http://www.opengl.org"><i>OpenGL</i></A> window. The quadstrip of interest approximates the <i>'Circular Annulus'</i> shape with internal radius <i>'rI'</i>, external radius <i>'rE'</i>, and center <i>'(xc,yc)'</i>. Note that the <i>'Circular Annulus'</i> shape is formed by all points <i>'( x(t), y(t) )'</i>, defined as follows:<p><pre>| rI * cos(t) | <= | x(t) - xc | <= | rE * cos(t) |, | rI * sin(t) | <= | y(t) - yc | <= | rE * sin(t) |</pre><p>for every <i>'t'</i> in <i>'[-pi,pi]'</i> and every <i>'R'</i> in <i>'[rI,rE]'</i> with <i>'rI'>0</i> and <i>'rE>0'</i>.<p>In other words, it is formed by all points, belonging to the intersection of two concentric <i>'Circle'</i> shapes of center <i>'(xc,yc)'</i> and of radius <i>'rI'</i> and <i>'rE'</i>, respectively.<p>This test also provides a very basic interaction mechanism with the user, which must provide the external radius <i>'rE'</i> and the internal radius <i>'rI'</i>, as well as the center coodinates <i>'(xc,yc)'</i>. The user can also:<p><ul> 
<li>decide to render only the points for the quadrilaterals in the quadstrip of interest by pressing the <i>'p'</i> key;</li>
<li>decide to render only the edges for the quadrilaterals (thus their <i>'wireframe versions'</i>) in the quadstrip of interest by pressing the <i>'l'</i> key;</li>
<li>decide to render completely the quadrilaterals (thus their <i>'filled versions'</i>) in the quadstrip of interest by pressing the <i>'f'</i> key;</li>
<li>increase the number of the quadrilaterals for the quadstrip of interest by pressing the <i>'+'</i> key;</li>
<li>decrease the number of the quadrilaterals for the quastrip of interest by pressing the <i>'-'</i> key.</li></ul>Likewise, the window of interest can be closed by pressing any among the <i>'Q'</i>, the <i>'q'</i>, and the <i>'Esc'</i> keys.<p>

In particular, this test consists of the <i>'Example-024-old'</i> executable file (in the <i>'bin'</i> folder), starting uniquely from the <i>'main.cpp'</i> C++ source file (in the <i>'src'</i> folder).<p><hr><p>
...
Specifically, the Win32 versions for the auxiliary libraries of interest in the <i>'../../depends'</i> folder are exploited for building the  <i>'Example-024 (Old Mode)'</i> Test. This implies that the <i>'Example-024-old.sln'</i> solution file is set up for including these auxiliary libraries, and it should not be modified, unless you are an expert. Furthermore, it is mandatory to enable the building for a generic Win32 platform for creating correctly the <i>'Example-024-old.exe'</i> Win32 executable file.<p>An interested user finds more details in the Official Documentation of the <i><A href="http://www.visualstudio.com/">Microsoft Visual Studio (Community Edition 2015)</A></i>.<p><hr><p>
