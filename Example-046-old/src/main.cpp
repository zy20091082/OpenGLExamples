...
int main(int argc,char **argv)
{
	...
	cout<<"hemisphere is known as the 'Hemisphere' shape. Broadly speaking, the 'Sphere'"<<endl;
	cout<<"\tshape describes a perfectly round geometrical object in the 3D space, that is the surface of a completely round ball. Like the 'Circle' shape, which ";
	cout<<"geometrically is an object in the 2D space, the 'Sphere' shape is defined"<<endl;
	cout<<"\tmathematically as the set of 3D points, that are at the same distance 'R' from a given point '(xc,yc,zc)'. The distance 'R' is the 'radius' of the ";
	...
	cout<<"shape) of 'radius' 'R' and 'center' '(xc,yc,zc)', such that its points are"<<endl<<"\texpressed as follows:"<<endl<<endl;
	cout<<"\tx(r,s) = xc + R * cos(r) * cos(s), y(r,s) = yc + R * sin(r), z(r,s) = zc + R * cos(r) * sin(s)"<<endl<<endl<<"\tfor any 'R>0', for any 'r' in ";
	cout<<"'[ 0, pi/3 ]', and for any 's' in '[ 0, 2*pi ]'."<<endl<<endl;
	cout<<"\tHere, the 'Dome' shape is approximated by a quad grid, formed by 'p' 'slices' around the Z-axis (similar to the longitudinal lines) and by 'q' 'stacks' ";
	cout<<"along the Z-axis (similar to the latitudinal lines). Specifically, the"<<endl;
...
void initialize() 
{
	...
	cout<<"' longitudinal slices (thus, the minimum numbers 'p' and 'q' as possible), as well as rotation angles 'Rx="<<Xangle<<"',";
	cout<<endl<<"\t'Ry="<<Yangle<<"', and 'Rz="<<Zangle<<"'."<<endl<<endl;
	cout.flush();
}
