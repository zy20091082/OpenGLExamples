 ...
 * 
 * main.cpp - the main function for the 'Example-049 (Old Mode)' example
 *******************************************************************************************************************************************************/
...
/// The main function for the <i>'Example-049 (Old Mode)'</i> example.
int main(int argc,char **argv)
{
	/* We initialize everything, and create a new window! */
	cout<<endl<<"\tThis is the 'Example-049' Example, based on the (Old Mode) OpenGL"<<endl<<endl;
	cout.flush();
	glutInit(&argc,argv);
	glutInitDisplayMode(GLUT_RGBA|GLUT_SINGLE);
	glutInitWindowPosition(0,0);
	glutInitWindowSize(500,500);
	glutCreateWindow("The 'Example-049' Example, based on the (Old Mode) OpenGL");
...
void manageKeys(unsigned char key, int x, int y)
{
	...
	switch (key)
	{
		...
		/* The key is 'H', thus we increase the number of the horizontal slices. */
		...
		/* The key is 'R', thus we increase the number of the radial slices. */
...
void initialize() 
{
	...
	Yangle=0.0;
	Zangle=0.0;
	cout<<"\tWe draw the wireframe version of the 'Truncated Cone' shape of interest. It is possible to::"<<endl<<endl;
...
void draw()
{
	...
	for(unsigned int h=0;h<height_slices_num;h++)
	{
		...
		/* Now, we complete the current horizontal slice! */
		...
		/* Now, we finalize the current horizontal slice! */
		glEnd();
	}
	...
	glFlush();
	cout<<"\tApproximated and drawn the 'Truncated Cone' shape by using "<<radial_slices_num<<" radial slices and "<<height_slices_num<<" horizontal slices, rotation angle around the x-axis: "<<Xangle<<", rotation angle around the y-axis: "<<Yangle;
